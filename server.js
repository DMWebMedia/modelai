...
// ── Virtual Try-On (VTON) — FASHN primary, IDM-VTON fallback ──────────────
// Best-in-class garment fidelity: actually warps the product garment onto
// the target person rather than synthesizing it.
async function generateVTON(personUrl, garmentUrl, category){
  // FASHN VTON
  try{
    const sub=await falQ('/fal-ai/fashn/tryon/v1.6',{
    ...
    if(sub.request_id){
      for(let i=0;i<60;i++){
        await new Promise(r=>setTimeout(r,2000));
        const sp=sub.status_url?sub.status_url.replace('https://queue.fal.run',''):`/fal-ai/fashn/tryon/v1.6/requests/${sub.request_id}/status`;
        const st=await falGet(sp);
        if(st.status==='COMPLETED'){
          const rp=sub.response_url?sub.response_url.replace('https://queue.fal.run',''):`/fal-ai/fashn/tryon/v1.6/requests/${sub.request_id}`;
          const res=await falGet(rp);
          const url=res?.images?.[0]?.url||res?.image?.url||res?.output?.images?.[0]?.url;
          if(url){console.log('[VTON FASHN]',category,'→',url.slice(0,60));return url;}
        }
        if(st.status==='FAILED') break;
      }
    }
    console.warn('[VTON] FASHN failed, falling back to IDM-VTON');
  }catch(e){console.warn('[VTON] FASHN error:',e.message);}

  // IDM-VTON fallback
  const idmCat = category==='one-pieces'?'dresses':category==='bottoms'?'lower_body':'upper_body';
  const sub=await falQ('/fal-ai/idm-vton',{
    human_image_url:personUrl,
    garment_image_url:garmentUrl,
    category:idmCat,
    description:'product garment from reference image',
  });
  if(!sub.request_id) throw new Error('VTON submit failed (both engines)');
  for(let i=0;i<60;i++){
    await new Promise(r=>setTimeout(r,2000));
    const sp=sub.status_url?sub.status_url.replace('https://queue.fal.run',''):`/fal-ai/idm-vton/requests/${sub.request_id}/status`;
    const st=await falGet(sp);
    if(st.status==='COMPLETED'){
      const rp=sub.response_url?sub.response_url.replace('https://queue.fal.run',''):`/fal-ai/idm-vton/requests/${sub.request_id}`;
      const res=await falGet(rp);
      const url=res?.image?.url||res?.images?.[0]?.url;
      if(url){console.log('[VTON IDM] →',url.slice(0,60));return url;}
    }
    if(st.status==='FAILED') throw new Error(st.error||'IDM-VTON failed');
  }
  throw new Error('VTON timed out');
}

// ── Inpaint accessories on top of a VTON result via GPT2 ─────────────────
// Accessories (bags, jewelry, belts, hats) aren't applied by VTON — we add
// them in a second pass using GPT2 edit with the accessory reference images.
async function inpaintAccessoriesGPT2(baseImageUrl, accessoryRefUrls, accDescription, aspectRatio, gptQuality){
  const refs=[baseImageUrl,...accessoryRefUrls];
  const prompt=sanitizeForGPT2(`Add the following accessories to the person in the FIRST reference image, exactly as shown in the other reference images: ${accDescription}. Reproduce each accessory's color, material, hardware, and placement EXACTLY. Keep the person, garment, pose, and background otherwise identical to the first image — only add the accessories.`);
  const sub=await falQ('/openai/gpt-image-2/edit',{prompt,image_urls:refs,quality:gptQuality||'medium',image_size:toGPT2Size(aspectRatio||'3:4'),content_moderation:'permissive'});
  if(!sub.request_id) throw new Error('Inpaint submit failed');
  for(let i=0;i<80;i++){
    await new Promise(r=>setTimeout(r,3000));
    ...
    if(st.status==='FAILED') throw new Error(st.error||'Inpaint failed');
  }
  throw new Error('Inpaint timed out');
}

// ── Detect garment category from Claude PIECES line ──────────────────────
function detectVTONCategory(piecesLine){
  const p=(piecesLine||'').toLowerCase();
  const isDress = /\b(dress|gown|jumpsuit|romper|bodysuit|playsuit|unitard|onesie|catsuit)\b/.test(p);
  const isTop = /\b(top|shirt|blouse|tee|t-shirt|tank|cami|camisole|sweater|hoodie|cardigan|blazer|jacket|coat|vest|bra|crop)\b/.test(p);
  const isBottom = /\b(pant|trouser|jean|short|skirt|legging|culotte|capri|joggers|sweatpant)\b/.test(p);
  return {
    isDress, isTop, isBottom,
    isSeparates: isTop && isBottom && !isDress,
    primary: isDress ? 'one-pieces' : isBottom && !isTop ? 'bottoms' : 'tops',
  };
}
...
// ── Pro tier orchestrator: VTON pipeline ──────────────────────────────────
async function generateProTier(item, modelAnchorUrls=[], batch=null){
  ...
  const cat=detectVTONCategory(F.pieces||F.prompt||'');
  const imgs=item.productImages||[];
  const sl=s=>(s||'').toLowerCase();
  const accImgs=imgs.filter(i=>sl(i.slot).startsWith('accessory'));
  const garmentImgs=imgs.filter(i=>!sl(i.slot).startsWith('accessory'));

  // VTON needs a person. Resolve in priority order:
  // 1. item.savedModelUrl
  // 2. modelAnchorUrls[0] (shot 0 result for shot>0, or pre-generated Flux)
  // 3. Flux base model from description
  // 4. For replaceModel=false: use the first product image directly (it shows the original model)
  let personUrl = modelAnchorUrls[0] || null;
  if(!personUrl){
    if(item.replaceModel){
      item.status='generating model';
      const modelDesc=item.modelDescText||GENDER[item.gender||'female']||GENDER.female;
      const poseHint = item.shotType==='back' ? ' facing away from camera, back view'
                      : item.shotType==='side' ? ' side profile view'
                      : '';
      try{ personUrl = await generateBaseModel(modelDesc+poseHint, item.aspectRatio); }
      catch(e){ console.warn('[pro] flux failed, falling back to GPT2:',e.message); return await generateGPT2(item, modelAnchorUrls); }
    } else if(garmentImgs.length){
      // Use product image as person ref (it already has the right model)
      personUrl = await uploadToFal(garmentImgs[0].base64, garmentImgs[0].mimeType);
    } else {
      return await generateGPT2(item, modelAnchorUrls);
    }
  }

  item.status='generating';

  // Pick the primary garment image (front view for front shots, back for back shots, etc.)
  // — already pre-filtered upstream by filterImagesForShot, so first non-accessory is correct
  if(!garmentImgs.length){
    // Accessory-only: just inpaint accessories on the person
    if(accImgs.length){
      const accUrls=await Promise.all(accImgs.map(i=>uploadToFal(i.base64,i.mimeType)));
      return await inpaintAccessoriesGPT2(personUrl, accUrls, F.acc||'the accessories shown', item.aspectRatio, item.gptQuality);
    }
    return await generateGPT2(item, modelAnchorUrls);
  }

  const primaryGarmentUrl=await uploadToFal(garmentImgs[0].base64, garmentImgs[0].mimeType);

  // Step A: VTON the primary garment
  let result;
  try{
    result = await generateVTON(personUrl, primaryGarmentUrl, cat.primary);
  }catch(e){
    console.warn('[pro] VTON failed:',e.message,'— falling back to GPT2');
    return await generateGPT2(item, modelAnchorUrls);
  }

  // Step B: if separates, run VTON again for the bottom piece
  if(cat.isSeparates && garmentImgs.length>=2){
    try{
      const bottomGarmentUrl=await uploadToFal(garmentImgs[1].base64, garmentImgs[1].mimeType);
      result = await generateVTON(result, bottomGarmentUrl, 'bottoms');
    }catch(e){console.warn('[pro] bottom VTON failed:',e.message);}
  }

  // Step C: inpaint accessories
  if(accImgs.length){
    try{
      const accUrls=await Promise.all(accImgs.map(i=>uploadToFal(i.base64,i.mimeType)));
      result = await inpaintAccessoriesGPT2(result, accUrls, F.acc||'accessories shown in reference', item.aspectRatio, item.gptQuality);
    }catch(e){console.warn('[pro] accessory inpaint failed:',e.message);}
  }

  return result;
}

// ── Premium tier: Pro + GPT2 in parallel, Claude judge picks winner ───────
async function generatePremiumTier(item, modelAnchorUrls=[], batch=null){
  const proPromise = generateProTier(item, modelAnchorUrls, batch).catch(e=>({error:e.message}));
  const gpt2Promise = generateGPT2({...item}, modelAnchorUrls).catch(e=>({error:e.message}));
  const [proResult, gpt2Result] = await Promise.all([proPromise, gpt2Promise]);

  const candidates=[];
  if(typeof proResult === 'string') candidates.push({name:'vton-pipeline', url:proResult});
  if(typeof gpt2Result === 'string') candidates.push({name:'gpt2-direct', url:gpt2Result});

  if(!candidates.length) throw new Error('Premium: both engines failed');
  ...
  return (candidates[idx] || candidates[0]).url;
}
...
async function processItem(batchId, itemId){
  ...
  try{
    ...
    //   low    → Standard (NB2): fast, garment-faithful, cheap
    //   medium → Pro (VTON pipeline): best fidelity for fashion
    //   high   → Premium (Pro + GPT2 parallel, Claude judge picks winner)
