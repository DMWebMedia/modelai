...
function buildPrompt(opts={}){
  ...
  const parts=[];
  // Model identity FIRST — most important for GPT Image 2
  if(modelLocked){
    parts.push('same model as the reference photo, identical face identical hair identical appearance');
  } else if(replaceModel){
    parts.push((modelDesc||GENDER[gender]||GENDER.female)+', completely different person from the product photo, ignore the model in the reference');
  ...
  return p.length>420?p.slice(0,417)+'...':p;
}
...
function buildPromptWithGarment(item, garmentDesc){
  ...
  // Filter angle-specific details
  let angleDesc=garmentDesc;
  if(shotKey==='front'||shotKey==='threeq'){
    // Remove back-only details from front prompt
    angleDesc=garmentDesc.replace(/,?\s*back:\s*(?:(?!front:|back:|sides:|accessories:|feet:).)+/gi,'').trim();
  } else if(shotKey==='back'){
    // Remove front-only details from back prompt
    angleDesc=garmentDesc.replace(/,?\s*front:\s*(?:(?!front:|back:|sides:|accessories:|feet:).)+/gi,'').trim();
  }
  // Always keep accessories and feet sections

  const parts=[];
  // 1. Garment lock — exact reproduction required
  parts.push('reproduce the exact garment and accessories described below — do not add, remove, or change anything');
  // 2. Angle-specific garment description from Claude vision
  ...
  if(item.modelLocked){
    ...
    const md=item.modelDescText||GENDER[item.gender||'female']||GENDER.female;
    parts.push(md+', completely different person from the product reference image');
  } else if(item.modelDescText){
    parts.push(item.modelDescText);
  } else {
    parts.push(GENDER[item.gender||'female']||GENDER.female);
  }
  // 5. Background — explicit
  const bg=item.bgOption==='custom'?item.bgCustom:(BG[item.bgOption]||'');
  if(bg) parts.push(bg);
  // 6. User scene prompt
  if(item.userPrompt) parts.push(item.userPrompt);
  // 7. Quality
  parts.push(REAL[item.realism||'ultra']||REAL.ultra);
